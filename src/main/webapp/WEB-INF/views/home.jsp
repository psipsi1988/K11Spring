...
<body>
	...
	<h2>스프링 MVC 시작하기</h2>
	<!-- 
		Spring MVC에서 이미지와 같은 리소스를 사용하기 위해 
...
4. 컨트롤러에서 외부파일을 읽어서 프로그램에 적용
	...
	4-4) 외부파일을 가져와서 addLast로 추가 후 내용 읽음
	4-5) 읽은 내용을 Model에 저장하거나 비즈니스로직에 직접사용함
 -->
...
5. 파일 다운로드를 위해 빈을 생성할 때는 미리
	...
	응답헤더와 스트림을 통해 파일을 다운로드하기 위한 로직으로 구성된다. 
-->
	<h3>파일업로드</h3>
	<li>
		<a href="./fileUpload/uploadPath.do" targt="_blank">
	...
	<h3>파일업로드</h3>
	<li>
		<a href="./fileUpload/uploadForm.do" targt="_blank">
			파일업로드
		</a>
	</li>
	...
	<li>
		<a href="./fileUpload/uploadList.do" targt="_blank">
				파일목록보기
		</a>
	</li>
	...
	-이와 같이 양쪽 모두 만족되어야 하나의 프로세스를 완료 처리할 수 있도록 
	해주는 기법을 트랜잭션이라고 한다. 
	-즉, 2개 이상의 쿼리를 하나의 커넥션으로 묶어 DB에 전송하고 이 과정에서 
	에러가 발생하는 경우 양쪽 모두를 원래의 상태로 되돌려놓는다. 
	
	절차1. 트랜잭션 매니저를 사용하는 경우
		1-1. 오라클 드라이버, SPRING-JDBC에 대한 의존 설정을 한다. 
		1-2. servelt-context.xml에서 빈을 생성한다. 
		1-3. TicketDAO의 빈을 컨트롤러에서 자동 주입받아 사용한다. 
		1-4. 2개 이상의 작업을 try~catch로 묶어 하나라도 오류가 발생하는 경우
			모든 작업을 follback시킨다. 
			모든 작업이 정상처리 되었다면 commit시킨다. 
	 -->
