...
public class RequestMappingController {
	...
	/*
	단순히 요청명만 매핑하는 경우에는 alert하method를 제외할 수 있으나 
	...
	ModelAndView객체
		: View로 전송할 데이터의 저장과 View를 호출하는
			두 가지 로직을 동시에 처리할 수 있는 
			사용법
				참조변수.setViewName("뷰의 경로 및 파일명")
					-> View설정
				참조변수.addObject("속성명", "속성값");
					-> View로 전달할 데이터 저장
				뷰를 호출할 때는 odelAndView참조변수를 return한다. 
	...
	@ModelAttribute 어노테이션
		: 뷰로 젠달되는 커맨드객체의 이름을 임의로 변경하고 싶을 때 사용한다. 
		아래 코드는 student -> si로 이름을 변경한 후 View로 데이터를 전달한다. 
	 */
	@RequestMapping("/requestMapping/modelAttribute")
	public String stuedentInfo(
