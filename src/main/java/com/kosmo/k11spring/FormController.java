...
사용자의 요청을 제일 먼저  받는 DispatcharServlet은 기본패키지로 지정된
com.kosmo.kqqspring을 스캔하여 컨트롤러 클래스를 찾는다. 
...
@RequestMapping
	: 요청명을 매핑한다. 
		요청명은 서블릿과 마찬가지로 컨텍스트루트를 제외한 
		나머지경로명으로 이루어진다.
		요청명의 매핑정보로 메소드를 호출하게 되므로 메소드명은 큰 의미가 없다. 
		개발자가 구분하기 좋은 정도의 이름을 설정하면 된다.  
...
public class FormController {
	...
	/*
	폼값받기2] @RequestParam 어노테이션으로 폼값 받기
	...
	/*
	폼값받기4] @PathVariable어노테이션으로 폼값 받기
		요청명./form 뒤에 붙은 값이 메소드에서 사용 가능한 파라미터가 된다. 
		아래의 경우 2개의 파라미터를 받아서 사용하게 된다. 
		단 웹브라우저는 요청명을 경로로 인식하므로 리소스(이미지 등)를 사용할 때는 
		경로에 주의해야 한다. 
