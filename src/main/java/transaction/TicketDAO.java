...
public class TicketDAO {
	...
	public void buyTicket(final TicketDTO dto) {
		...
		/*
		테스트시에는 아래 두 개의 업무처리 로직을 try문 밖으로 이동한 후
		구매를 해본다. 
		6장 이상인 경우 pay테이블에는 insert
		ticket테이블에는 제약 조건 위배로 insert되지 않는다. 
		 */
		try {
			//티켓 결제 금액에 대한 DB처리
			...
			/*
			티켓 구매 개수가 5 이하인 경우 정상 리되어 모든 작업이 commit된다. 
			 */
			...
			/*
			티켓 구매 개수가 6 이상인 경우 예외가 발생되어 모든 작업은 
				취소(rollback) 처리된다. 
			 */
			System.out.println("제약조건을 위배하여 "
					+ "카드결제와 티켓 구매 모두가 취소되었습니다. ");
